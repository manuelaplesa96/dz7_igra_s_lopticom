...
void rezultat() {
  ...
  fill(boja1);
    ...
    textSize(15);
    text("POVRATAK", 340, 49);
    
    if(mousePressed)
    {
      if(prelazak(267, 30, 140, 40))
      {
        prozor = 0;
        setup();
      }
...
void setup(){
  ...
  size(700, 800);
  pozadina1 = loadImage("pozadina1.jpg");
  pozadina1.resize(700, 800);
  pozadina2 = loadImage("pozadina2.jpg");
  pozadina2.resize(700, 800);
...
void draw(){
  ...
  if(prozor == 1)
  {
    if(igra)
    {
      background(pozadina1);
    
      fill(boja1);
      textSize(30);
      text("Rezultat: " + rezultat, 100, 40);
      
      //crvena loptica koja je uvijek tamo gdje je miš
      //fill(255, 0, 0);
      //ellipse(mouseX, mouseY, radijus, radijus);
      image(player, mouseX-20,mouseY,50,50);
  ...
  if(prozor == 5)
  {
    ...
    textAlign(LEFT);
    fill(boja2);
    text("U ovoj igrici, vi ste Mickey Mouse. \nPomičući miša, mičete Mickeya. \nCilj je dotaknuti što više zelenih. \nSa svakom dotaknutom zelenom lopticom, \nbroj plavih se povećava.\nIgra je gotova kada dotaknete plavu lopticu.", 25, 125);
   
    fill(boja1);
    textSize(30);
    text("Igrica za dva igrača", 150, 400);
    textSize(20);
    textAlign(LEFT);
    fill(boja2);
